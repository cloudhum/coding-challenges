...
function barista(coffees){
    //sort coffees array 
    let sortedArray = coffees.sort(function(a, b){return a - b}); 
    
    //track wait for each individual customer and the combined wait time
    let wait = 0;
    let combinedWait = 0; 
    
    for (let i = 0; i < sortedArray.length; i++){
      //first customer doesn't have to wait for machine to be cleaned
      if (i == 0) {
        wait = sortedArray[i];
        combinedWait += wait; 
        continue;
