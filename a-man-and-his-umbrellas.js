...
function minUmbrellas(weather) {
    let hasUmbrellaHome = 0;
    let hasUmbrellaWork = 0;
    let minUmbrellas = 0;

    for (let i = 0; i < weather.length; i++) {

        if (weather[i] == "clear" || weather[i] == "sunny" || weather[i] == "cloudy" || weather[i] == "overcast" || weather[i] == "windy") {
            if (((weather[i - 1] == "rainy") || (weather[i - 1] == "thunderstorms")) && (i % 2 == 0)) hasUmbrellaHome++;
            if (((weather[i - 1] == "rainy") || (weather[i - 1] == "thunderstorms")) && (i % 2 !== 0)) hasUmbrellaWork++;
        }

        if (weather[i] == "rainy" || weather[i] == "thunderstorms") {
            if ((weather[i - 1] !== "rainy") && (weather[i - 1] !== "thunderstorms")) {
                if (i % 2 == 0) {
                    if (hasUmbrellaHome >= 1) hasUmbrellaHome--;
                    else minUmbrellas++;
                }

                else {
                    if (hasUmbrellaWork >= 1) hasUmbrellaWork--;
                    else minUmbrellas++;
                }
            }
        }
